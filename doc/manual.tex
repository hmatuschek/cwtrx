...
\subtitle{Board rev. 2, Firmware rev. 2.5}
...
\begin{figure}[!ht]
 ...
 \begin{tikzpicture}[node distance=2.3cm,
  ...
  \node (type) [submenu, left of=quick] {CW key};
  \node (rev) [submenu, left of=type] {Pad.rev};
  \node (mode) [submenu, left of=rev] {Iambic}; 
  \node (tone) [submenu, left of=mode] {CW Tone}; 
  \node (level) [submenu, left of=tone] {CW Level}; 
  \node (text) [submenu, left of=level] {CW Text}; 
  \node (clr) [submenu, below of=text] {Clr. CWT};
...
\paragraph{Key type}
The key type specifies the type of keyer you use. Possible options are \emph{straight key}, \emph{paddle}and \emph{iambic}. The TRX switches automatically to \emph{straight key} if the TRX detects a mono-plug at startup.

\paragraph{Paddle reverse}
If enabled, the meaning of the two paddles are reversed. That is, the left paddle will produce a series of \emph{dahs} and the right one produces a series of \emph{dits}.

\paragraph{Iambic mode}
Specifies either \emph{iambic A} or \emph{iambic B} modes for the automatic keyer if the type is set to \emph{iambic}.
...
\includepdf[landscape=true]{fig/drill_back.pdf}
\end{document}